...
-- ----------------------------
-- Table structure for ai_org
-- ----------------------------
CREATE TABLE IF NOT EXISTS `ai_org` (
                          `org_id` INT NOT NULL COMMENT '单位ID，用于关联指标数据',
                          `org_name` VARCHAR(100) NOT NULL COMMENT '组织全称',
                          `org_byname` VARCHAR(50) COMMENT '报表常用单位简称',
                          `org_description` TEXT COMMENT '报表常用的划分标识，如哪些是省分或北10省（多个时用逗号分隔）',
                          `org_former_name` TEXT COMMENT '单位曾经变更过的名称（多个时用逗号分隔）',
                          PRIMARY KEY (`org_id`)
);
COMMENT ON TABLE ai_org is '组织信息表';
-- ----------------------------
-- Records of ai_org
-- ----------------------------
-- （插入语句与MySQL完全兼容，无需修改）
...
CREATE TABLE IF NOT EXISTS `ai_report` (
                             `report_id` INT NOT NULL COMMENT '单张表的ID,用于关联指标数据',
                             `cycle_name` VARCHAR(100) NOT NULL COMMENT '一套报表的分类名称，如产互还原月报',
                             `cycle_physics_name` VARCHAR(100) NOT NULL COMMENT '物理表名称',
                             `report_name` VARCHAR(100) NOT NULL COMMENT '对应报表ID的具体表名',
                             `target_info` TEXT COMMENT '对应报表下有哪些指标（多个时用逗号分隔）',
                             PRIMARY KEY (`report_id`)
);
COMMENT ON TABLE ai_report is '报表信息表';
-- ----------------------------
-- Table structure for ai_target_rc_date
-- ----------------------------
CREATE TABLE IF NOT EXISTS `ai_target_rc_date` (
                                     `id` INT GENERATED BY DEFAULT AS IDENTITY COMMENT '主键id', -- H2的自增语法
                                     `year_month` VARCHAR(10) COMMENT '指标值对应的时间维度',
                                     `org_id` VARCHAR(20) COMMENT '同组织信息表',
                                     `report_id` VARCHAR(20) COMMENT '同报表信息',
                                     `target_name` VARCHAR(100) COMMENT '指标的名称',
                                     `target_value` DECIMAL(18, 4) COMMENT '对应账期、单位及报表的指标值',
                                     PRIMARY KEY (`id`)
);
COMMENT ON TABLE ai_target_rc_date is '人才强企各指标月报';
-- ----------------------------
-- Table structure for ai_target_yg_date
-- ----------------------------
CREATE TABLE IF NOT EXISTS `ai_target_yg_date` (
                                     `id` INT GENERATED BY DEFAULT AS IDENTITY COMMENT '主键id', -- 自增语法调整
                                     `year_month` VARCHAR(10) COMMENT '指标值对应的时间维度',
                                     `org_id` VARCHAR(20) COMMENT '同组织信息表',
                                     `report_id` VARCHAR(20) COMMENT '同报表信息',
                                     `target_name` VARCHAR(100) COMMENT '指标的名称',
...
COMMENT ON TABLE ai_target_rc_date is '产互还原月报';
